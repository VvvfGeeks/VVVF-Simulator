...
例如，设置 `载波频率` 参数栏的值为 500，并将该项的值设置为 50，这意味着在基于 500Hz 载波频率的情况下可以在可以在 550Hz 到 450Hz 之间变化（±50Hz）。`间隔(秒)` 指定载波频率可以以秒为单位改变的频率。如果您想在一定的基波频率范围内让载波频率升高或降低，则将 `载波模式` 栏将 `恒定频率` 更改为 `可变频率` 。更改后，可以在 `载波频率` 一栏中看到 `变化类型` 下拉菜单，以及 `开始时基波频率` 、 `结束时基波频率` 、`开始时的值` 和 `结束时的值` 等参数栏。<br>
